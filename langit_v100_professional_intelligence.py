#!/usr/bin/env python3
"""
LANGIT v100 Professional Local Atmosphere Intelligence
==========================
...
Usage in repository root:
  python langit_v100_professional_intelligence.py --root outputs --public-base-url https://marcooo20-d.github.io/weather-forecast
  python langit_v100_professional_intelligence.py --root outputs --verify-only
"""
...
def decision_sentence(location: str, day: Dict[str, Any], short: bool = False) -> str:
    ...
    peak = text(day.get("peak_rain_hour"), "—").replace(":", ".")
    heat = num(day.get("max_heat_c"), None)

    if c == "limited":
        return "Data prakiraan belum lengkap. Gunakan sebagai indikasi awal dan tetap periksa kondisi setempat."
    if c == "danger":
        return f"Risiko cuaca tinggi. Hindari aktivitas luar ruang pada sekitar pukul {peak} WIB."
    if c == "rain":
        return f"Peluang hujan cukup tinggi. Siapkan payung atau jas hujan, terutama sekitar pukul {peak} WIB."
    if heat is not None and heat >= 36:
        return "Cuaca relatif aman dari hujan, namun suhu terasa cukup panas pada siang hari."
    if c == "watch":
        return f"Cuaca masih memungkinkan untuk aktivitas luar ruang. Tetap pantau kondisi sekitar pukul {peak} WIB."
    return f"Cuaca hari ini relatif aman untuk aktivitas luar ruang. Peluang hujan tertinggi {p} sekitar pukul {peak} WIB."


def short_activity_advice(day: Dict[str, Any]) -> List[Tuple[str, str, str, str]]:
    c = day.get("risk_class", "watch")
    peak = text(day.get("peak_rain_hour"), "—").replace(":", ".")
    win = ", ".join(day.get("safe_windows") or ["pagi hingga siang"])
    heat = num(day.get("max_heat_c"), 0) or 0

    if c in {"danger", "rain"}:
        return [
            ("Motor", "Waspada", f"Siapkan jas hujan. Hindari perjalanan pada sekitar pukul {peak} WIB.", "rain"),
            ("Jalan kaki", "Dipantau", "Pilih rute yang memiliki tempat berteduh.", "watch"),
            ("Jemur pakaian", "Tidak ideal", "Disarankan hanya pagi hari dan jangan ditinggal terlalu lama.", "watch"),
            ("Olahraga", "Dipantau", "Pilih aktivitas ringan atau indoor.", "watch"),
            ("Aktivitas luar ruang", "Waspada", "Siapkan rencana cadangan di dalam ruangan.", "rain"),
            ("Foto/city walk", "Dipantau", "Lindungi perangkat jika awan mulai gelap.", "watch"),
        ]
    if c == "limited":
        return [
            ("Motor", "Cek manual", "Data terbatas. Periksa kondisi langit sebelum berangkat.", "limited"),
            ("Jalan kaki", "Terbatas", "Masih memungkinkan jika kondisi sekitar mendukung.", "limited"),
            ("Jemur pakaian", "Terbatas", "Jangan ditinggal terlalu lama.", "limited"),
            ("Olahraga", "Terbatas", "Pilih durasi pendek dan pantau kondisi setempat.", "limited"),
            ("Aktivitas luar ruang", "Terbatas", "Tetap fleksibel karena data belum lengkap.", "limited"),
            ("Foto/city walk", "Terbatas", "Cek kondisi langit setempat.", "limited"),
        ]
    if heat >= 36:
        return [
            ("Motor", "Aman", "Perhatikan panas saat siang hari.", "safe"),
            ("Jalan kaki", "Dipantau", "Lebih nyaman pagi atau sore hari.", "watch"),
            ("Jemur pakaian", "Aman", "Disarankan pagi hingga siang.", "safe"),
            ("Olahraga", "Dipantau", "Disarankan pagi hari. Hindari siang yang terasa panas.", "watch"),
            ("Aktivitas luar ruang", "Dipantau", "Masih memungkinkan, tetapi kurangi aktivitas berat saat siang.", "watch"),
            ("Foto/city walk", "Aman", "Pilih waktu pagi atau sore.", "safe"),
        ]
    return [
        ("Motor", "Aman", "Tetap siapkan pelindung hujan jika keluar sore.", "safe"),
        ("Jalan kaki", "Aman", f"Aman, terutama pada {win}.", "safe"),
        ("Jemur pakaian", "Aman", "Disarankan pagi hingga siang.", "safe"),
        ("Olahraga", "Aman", "Disarankan pagi atau sore hari.", "safe"),
        ("Aktivitas luar ruang", "Aman", "Masih memungkinkan. Tetap pantau kondisi sore.", "safe"),
        ("Foto/city walk", "Aman", "Pilih waktu dengan cahaya yang cukup.", "safe"),
    ]
...
CSS = r'''
:root{--bg:#f4f8fc;--panel:#ffffff;--panel2:#f8fbff;--line:#dbe6f1;--text:#102033;--muted:#66768a;--blue:#1d4ed8;--green:#15803d;--amber:#b7791f;--orange:#c2410c;--red:#b91c1c;--limited:#6b7280;--shadow:0 10px 30px rgba(16,32,51,.07)}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,Roboto,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;letter-spacing:-.01em;line-height:1.5}a{text-decoration:none;color:inherit}.top{position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px clamp(16px,4vw,70px);background:rgba(255,255,255,.95);backdrop-filter:blur(14px);border-bottom:1px solid var(--line)}.brand{display:flex;align-items:center;gap:12px}.logo{width:38px;height:38px;border-radius:12px;background:linear-gradient(135deg,#1d4ed8,#38bdf8);box-shadow:inset 0 0 0 1px rgba(255,255,255,.55)}.brand b{display:block;font-size:18px}.brand small{display:block;color:var(--muted);font-size:12px;margin-top:1px}.navs{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}.nav,.btn{border:1px solid transparent;background:transparent;border-radius:999px;padding:8px 12px;font-size:13px;font-weight:650;color:#334155}.nav.active{background:#e8f0ff;color:var(--blue);border-color:#c9dcff}.btn{border-color:var(--line);background:#fff}.btn.primary{background:var(--blue);border-color:var(--blue);color:#fff}.wrap{width:min(1180px,calc(100% - 32px));margin:0 auto;padding:28px 0 56px}.hero-grid{display:grid;grid-template-columns:minmax(0,1.5fr) 330px;gap:18px}.hero{min-height:210px;border-radius:22px;border:1px solid var(--line);padding:28px;background:#fff;box-shadow:var(--shadow);position:relative;overflow:hidden}.hero:after{content:"";position:absolute;right:-90px;bottom:-110px;width:260px;height:260px;border-radius:50%;background:linear-gradient(135deg,rgba(29,78,216,.08),rgba(56,189,248,.18))}.chips{display:flex;gap:8px;flex-wrap:wrap;position:relative;z-index:1}.chip{font-size:12px;font-weight:650;background:#f1f6fc;border:1px solid var(--line);padding:7px 10px;border-radius:999px;color:#334155}.hero h1{position:relative;z-index:1;margin:28px 0 10px;font-size:clamp(34px,5vw,58px);line-height:1.03;color:var(--text)}.hero p{position:relative;z-index:1;margin:0;color:#2a3b4d;font-size:clamp(15px,1.8vw,18px);max-width:760px}.side{display:grid;grid-template-columns:1fr 1fr;gap:12px}.tile,.panel,.decision-main,.kpi,.card,.hour,.stat,.map-card{background:var(--panel);border:1px solid var(--line);box-shadow:var(--shadow)}.tile{border-radius:18px;padding:18px;min-height:102px}.tile.main{grid-column:1/-1;min-height:132px}.tile span,.kpi span,.stat span{display:block;color:var(--muted);font-size:12px;font-weight:700}.tile strong{display:block;font-size:34px;margin:8px 0;color:var(--text)}.tile b{display:block;font-size:24px;margin:8px 0;color:var(--text)}.tile p,.kpi small{margin:0;color:var(--muted)}.notice{margin:14px 0 17px;padding:10px 12px;border-radius:14px;border:1px solid #fde68a;background:#fffbeb;color:#6b4e12;font-size:12px;font-weight:500}.decision{display:grid;grid-template-columns:minmax(0,1.35fr) 330px;gap:16px;margin-top:16px}.decision-main{border-radius:22px;padding:24px;min-height:190px;display:flex;flex-direction:column;justify-content:space-between}.badge{display:inline-flex;width:max-content;padding:6px 10px;border-radius:999px;font-size:13px;font-weight:750}.safe,.watch,.rain,.danger,.limited{color:inherit}.badge.safe{color:#166534;background:#dcfce7}.badge.watch{color:#8a5a00;background:#fef3c7}.badge.rain{color:#9a3412;background:#ffedd5}.badge.danger{color:#991b1b;background:#fee2e2}.badge.limited{color:#374151;background:#f3f4f6}.decision-main h2{font-size:clamp(26px,3.4vw,42px);line-height:1.08;margin:18px 0 0;color:var(--text)}.decision-main p{margin:16px 0 0;color:var(--muted)}.kpis{display:grid;grid-template-columns:1fr 1fr;gap:11px}.kpi{border-radius:16px;padding:16px}.kpi strong{display:block;font-size:24px;margin:8px 0 3px;color:var(--text)}.panel{margin-top:20px;padding:22px;border-radius:20px}.head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin-bottom:18px}.head h2{font-size:24px;margin:0;color:var(--text)}.head p{margin:0;color:var(--muted);font-size:13px}.timeline{display:grid;grid-template-columns:repeat(var(--n),1fr);align-items:end;gap:10px;min-height:135px}.bar{text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:end;gap:8px}.bar b{font-size:13px;color:var(--text)}.bar small{font-size:12px;color:var(--muted)}.bar .v{width:100%;min-height:8px;border-radius:10px 10px 4px 4px;background:var(--c);opacity:.9}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:16px}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}.card{border-radius:18px;padding:18px}.card h3{margin:0 0 8px;font-size:21px;color:var(--text)}.card p{margin:0;color:#334155}.card .micro{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:14px}.stat{border-radius:14px;padding:11px;background:#f8fbff;box-shadow:none}.stat b{display:block;font-size:18px;margin-top:5px;color:var(--text)}.activities{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.activity{min-height:130px;border-left:4px solid var(--accent);border-radius:16px;padding:16px;background:#fff;border-top:1px solid var(--line);border-right:1px solid var(--line);border-bottom:1px solid var(--line)}.activity h3{margin:0 0 8px;font-size:18px;color:var(--text)}.activity b{display:block;font-size:16px;color:var(--text)}.activity p{color:#334155}.focus{font-size:12px;color:var(--muted);font-weight:650;margin-top:12px}.hours{display:grid;gap:9px}.hour{display:grid;grid-template-columns:78px minmax(0,1.4fr) repeat(4,minmax(94px,.55fr));gap:9px;align-items:center;border-radius:16px;padding:12px;border-left:4px solid var(--accent);box-shadow:none}.time{font-size:19px;font-weight:750}.hour h3{margin:0;font-size:16px;color:var(--text)}.hour p{margin:2px 0 0;color:var(--muted);font-size:12px}.hbox{border-radius:12px;padding:10px;background:#f8fbff;border:1px solid var(--line)}.hbox b{display:block;font-size:16px;color:var(--text)}.hbox span{font-size:11px;color:var(--muted)}.map-frame{width:100%;height:430px;border:1px solid var(--line);border-radius:18px;background:#eef4fa}.map-card{border-radius:20px;padding:0;border:0;box-shadow:none}.actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:12px}.subtle{color:var(--muted)}details.clean{margin-top:14px;border:1px solid var(--line);border-radius:16px;background:#fff;padding:14px}details.clean summary{cursor:pointer;font-weight:700}table{width:100%;border-collapse:collapse;min-width:620px}th,td{text-align:left;padding:12px;border-bottom:1px solid var(--line)}th{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em}.tablewrap{overflow:auto}.footer{margin:28px 0 0;text-align:center;color:var(--muted);font-size:12px}.mini-footer{display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin-top:20px}.mini-footer a{color:var(--blue);font-size:12px}.visual-note{font-size:12px;color:var(--muted)}@media(max-width:980px){.hero-grid,.decision,.grid2,.grid3,.grid4{grid-template-columns:1fr}.side,.kpis{grid-template-columns:1fr 1fr}.activities{grid-template-columns:1fr 1fr}.hour{grid-template-columns:70px 1fr 1fr 1fr}.hour .hbox:nth-last-child(-n+2){display:none}.top{align-items:flex-start;flex-direction:column}}@media(max-width:640px){.wrap{width:calc(100% - 22px)}.hero{min-height:190px;padding:22px;border-radius:18px}.hero h1{font-size:34px}.side,.kpis,.activities,.card .micro{grid-template-columns:1fr 1fr}.timeline{display:flex;overflow:auto;min-height:125px}.bar{min-width:54px}.bar .v{width:42px}.hour{grid-template-columns:62px 1fr}.hour .hbox{display:none}.hour .rainbox,.hour .riskbox{display:block}.panel{padding:16px}.map-frame{height:360px}.nav{padding:8px 10px}}
...
def nav(api: Dict[str, Any], active: str, root: bool = False) -> str:
    if root:
        items = [("Ringkasan Lokasi", "index.html", "locations"), ("Peta", "langit_portal_map.html", "map")]
        subtitle = "Prakiraan Cuaca Lokal"
        href = "index.html"
    else:
...
<section class="decision">
  <article class="decision-main">
    ...
    <p>{esc(day.get('date_label'))}. {esc(rain_phrase(day))}. Waktu terbaik: {esc(windows_text)}.</p>
  </article>
  <aside class="kpis">
    ...
    {kpi('Waktu terbaik', windows_text, 'aktivitas')}
...
def map_page(title: str, geojson: Dict[str, Any], back_href: str, full: bool = True) -> str:
    ...
    css = r'''
html,body,#map{height:100%;margin:0;background:#eef4fa;color:#102033;font-family:Inter,Roboto,system-ui,-apple-system,Segoe UI,sans-serif}.hud{position:absolute;z-index:900;left:18px;top:18px;width:min(360px,calc(100% - 36px));padding:16px;border-radius:16px;background:rgba(255,255,255,.96);border:1px solid #dbe6f1;box-shadow:0 10px 30px rgba(16,32,51,.12)}.hud h1{font-size:20px;line-height:1.1;margin:0 0 8px}.hud p{margin:0;color:#66768a;font-size:13px}.btn{display:inline-flex;margin-top:14px;padding:8px 11px;border-radius:10px;background:#1d4ed8;color:#fff;text-decoration:none;font-weight:700;font-size:13px}.timebar{position:absolute;z-index:900;left:50%;bottom:20px;transform:translateX(-50%);display:flex;gap:8px;max-width:calc(100% - 34px);overflow:auto;padding:10px;border-radius:999px;background:rgba(255,255,255,.96);border:1px solid #dbe6f1;box-shadow:0 10px 30px rgba(16,32,51,.12)}.tbtn{border:1px solid #dbe6f1;background:#fff;color:#334155;border-radius:999px;padding:8px 12px;font-weight:700;cursor:pointer}.tbtn.active{background:#1d4ed8;border-color:#1d4ed8;color:white}.legend{position:absolute;right:18px;bottom:20px;z-index:901;background:rgba(255,255,255,.96);border:1px solid #dbe6f1;border-radius:14px;padding:10px;font-size:12px;box-shadow:0 10px 30px rgba(16,32,51,.12)}.legend div{display:flex;gap:8px;align-items:center;margin:5px}.dot{width:10px;height:10px;border-radius:50%;background:var(--c)}.fallback{position:absolute;inset:0;display:grid;place-items:center;color:#66768a}.leaflet-popup-content-wrapper,.leaflet-popup-tip{background:#fff;color:#102033}.leaflet-popup-content b{font-size:15px}@media(max-width:700px){.hud{left:12px;top:12px}.legend{right:12px;bottom:92px}.timebar{bottom:18px}}
'''
    js = r'''
const data = __DATA__;
...
try{
  ...
  L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,noWrap:true,bounds:[[-11.25,94],[6.45,141.25]],attribution:'&copy; OpenStreetMap'}).addTo(map);
...
def map_embed(api: Dict[str, Any], href: str = "langit_map_room.html") -> str:
    return f'<section class="panel"><div class="head"><h2>Peta risiko</h2><p>Warna berubah per jam.</p></div><div class="map-card"><iframe class="map-frame" src="{esc(href)}" loading="lazy"></iframe><div class="actions"><a class="btn primary" href="{esc(href)}">Buka Peta</a><a class="btn" href="langit_location.geojson">GeoJSON</a></div></div></section>'
...
function decidePlanner(){{
  ...
  else if(r>=25 || p>=25){{msg='Masih memungkinkan, pantau'; why='Masih aman bersyarat.';}}
