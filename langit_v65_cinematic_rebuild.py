...
.hour-row {
  display: grid;
  grid-template-columns: 80px 1.5fr repeat(4, minmax(90px, 0.5fr));
  gap: 16px;
  align-items: center;
  padding: 16px 24px;
...
@media (max-width: 768px) {
  .hero { padding: 40px 16px 80px; min-height: auto; }
  .hero-title { font-size: 40px; }
  .hero-metrics { flex-direction: column; align-items: center; width: 100%; }
  .hero-metric { width: 100%; max-width: 320px; }
  .container { width: calc(100% - 32px); }
  .section { padding: 48px 0; }
  .nav-bar { padding: 0 20px; height: 64px; }
  .nav-links { gap: 4px; }
...
(function(){
  ...
  function initParticles() {
    particles = [];
    let count = 40;
    if (state.activeLayer === 'rain') count = 80;
    else if (state.activeLayer === 'cloud') count = 15;
    else if (state.activeLayer === 'humidity') count = 30;
