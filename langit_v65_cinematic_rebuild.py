...
@media(max-width:1279px){
  .hud{width:min(300px,calc(100% - 48px))}
}
@media(min-width:768px) and (max-width:1023px){
  .hud{width:260px;padding:18px}
  .hud h1{font-size:20px}
  .legend{width:130px;padding:12px}
  .timeline-container{width:calc(100% - 32px);padding:10px 14px}
}
@media(max-width:767px){
  .hud{left:0;top:auto;bottom:0;width:100%;max-height:42vh;overflow-y:auto;padding:16px 16px 20px;border-radius:24px 24px 0 0;
    box-shadow:0 -12px 40px rgba(0,0,0,0.5);animation:slideUp 0.5s cubic-bezier(0.16,1,0.3,1) both;
    -webkit-overflow-scrolling:touch;transition:max-height 0.35s ease}
  .hud.collapsed{max-height:60px;overflow:hidden}
  .hud-collapse-toggle{display:flex;align-items:center;justify-content:center;width:100%;height:44px;margin:-12px auto 0;cursor:pointer;background:none;border:none;padding:0}
  .hud-collapse-bar{width:36px;height:4px;border-radius:2px;background:rgba(255,255,255,0.2);transition:background 0.3s ease}
  .hud-collapse-toggle:hover .hud-collapse-bar{background:rgba(255,255,255,0.4)}
  .hud h1{font-size:18px;margin:8px 0 4px}
  .hud-stats-grid{grid-template-columns:1fr 1fr;gap:8px}
  .hud-stat-val{font-size:15px}
  .hud-stat-lbl{font-size:9px}
  .parameter-tabs{margin-top:12px}
  .param-tab{padding:10px 0;font-size:11px;min-height:40px}
  .timeline-container{bottom:0;left:0;width:100%;border-radius:0;padding:8px 12px;
    transform:translateX(0);animation:none}
  .time-scrubber{scroll-snap-type:x mandatory}
  .time-pill{scroll-snap-align:center;min-width:80px;padding:10px 14px}
  .play-btn{width:44px;height:44px}
  .legend.collapsed{max-height:30px;overflow:hidden;padding:8px 10px}
  .legend.collapsed > :not(.legend-toggle){display:none!important}
  .legend-toggle{display:block;cursor:pointer;background:none;border:none;color:#fff;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;padding:0;font-family:inherit}
  .btn{min-height:44px;padding:12px 20px}
}
@media(max-width:479px){
  .hud{padding:12px 12px 16px;max-height:38vh}
  .hud h1{font-size:16px}
  .hud-stats-grid{gap:6px}
  .hud-stat-box{padding:8px 10px}
  .hud-stat-val{font-size:14px}
  .time-pill{min-width:70px;padding:8px 10px;font-size:12px}
  .legend{font-size:9px;min-width:90px}
}
@media(prefers-reduced-motion:reduce){
  .pulse-ring,.radar-ping{display:none}
...
function updateLegend() {
  ...
  legendEl.innerHTML = `<button class="legend-toggle" aria-label="Toggle Legend">Legenda ${isCollapsed ? '▴' : '▾'}</button>` + legendHtml;
}
...
function togglePlay() {
  ...
  if (playInterval) {
    ...
    playInterval = setInterval(() => {
      ...
      scrollPillIntoView(activeTimeIndex);
    }, 2500);
  }
}
...
window.addEventListener('message', (e) => {
  ...
  } else if (e.data && e.data.type === 'invalidateSize') {
...
if (legendEl) {
  legendEl.addEventListener('click', (e) => {
    ...
    if (btn) {
      ...
      const isCollapsed = legendEl.classList.contains('collapsed');
      btn.textContent = `Legenda ${isCollapsed ? '▴' : '▾'}`;
    }
  });
}
